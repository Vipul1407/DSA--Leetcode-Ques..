...
public:
    ...
    {
        return true;//as we will try to play optimally always..
    }
};
...
bool stoneGame(vector<int> &piles)
{
    ...
    for (int i = n - 2; i >= 0; i--)
    {
        // starting from 2nd row..i.e j=1
        // or will be accepted if we start from j=i+1
        for (int j = 1; j < n; j++)
        {
...
bool stoneGame(vector<int> &piles)
{
    ...
    return next[n - 1]; // returning n-1 th not 0
}
*/