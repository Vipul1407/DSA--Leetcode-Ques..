...
public:
    ...
    long long minimumTotalDistance(vector<int>& robot, vector<vector<int>>& fact) 
    {
        ...
        vector<long long>curr(fs+1,1e15);
        vector<long long>next(fs+1,1e15);
        for(int f=fs;f>=0;f--)
        {
            next[f]=0;
        }
        for(int r=rs-1;r>=0;r--)
        {
            ...
            {
                long long inc= abs(robot[r]-position[f])+ next[f+1];
                long long notinc= curr[f+1];
                curr[f]= min(inc,notinc);
            }
            next= curr;
        }
        return next[0];
